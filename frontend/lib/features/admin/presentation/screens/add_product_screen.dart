...
class AddProductScreen extends StatefulWidget {
  ...
  final Future<String> Function({required List<int> bytes, required String fileName}) onUploadImage;
...
class _AddProductScreenState extends State<AddProductScreen> {
                ...
                children: [
                  ...
                  Align(
                    ...
                    child: OutlinedButton.icon(
                      ...
                      onPressed: _isSaving || _isUploadingImage ? null : _pickImageFromDevice,
                      icon: const Icon(Icons.photo_library_outlined),
                      label: Text(_isUploadingImage ? 'Uploading image...' : 'Select from device'),
                    ),
                  ),
  ...
  Future<void> _pickImageFromDevice() async {
    try {
      ...
      // Upload selected file immediately and store only the URL in product data.
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('File picker plugin is not loaded. Stop and re-run the app after flutter pub get.'),
        ),
      );
    } catch (error) {
      if (!mounted) {
...
String _friendlyUploadErrorMessage(Object error) {
  ...
  if (error is StateError && error.message.toString().contains('APP_API_BASE_URL')) {
  ...
  if (error is CommerceApiException) {
    ...
    if (error.statusCode == 413 || message.contains('too large') || message.contains('file size')) {
