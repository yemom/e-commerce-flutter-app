...
/// Handles Order state and actions.
class OrderNotifier extends StateNotifier<OrderState> {
  ...
  Future<void> loadOrders({String? branchId, OrderStatus? status}) async {
    // Keep selected status in state so UI chips and fetched data stay synchronized.
  ...
  Future<void> confirmOrder(Order order) async {
    // Convenience wrapper used by call sites that do not need the returned value.
  ...
  Future<void> updateStatus({required String orderId, required OrderStatus status}) async {
    // Update local cache optimistically with authoritative server response.
  ...
  Future<void> verifyPayment({
    ...
    // Payment verification updates order-level payment state.
