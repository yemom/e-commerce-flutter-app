...
class PaymentGatewayEnvironmentConfig {
  ...
  factory PaymentGatewayEnvironmentConfig.fromEnvironment() {
    // Support both new CBE key names and legacy CHAPA-compatible names.
    ...
    // Clamp timing values to avoid overly aggressive polling or very long waits.
...
class TelebirrPaymentGatewayAdapter extends PaymentGatewayAdapter {
  ...
  Future<PaymentGatewayResult> charge(PaymentGatewayRequest request) async {
    ...
    // Start a Telebirr charge and collect transaction reference for later verification.
  ...
  Future<PaymentGatewayResult> verify(PaymentGatewayVerificationRequest request) async {
    // Verify current transaction state with Telebirr verify endpoint.
  ...
  Uri _resolveTelebirrEndpoint() {
    // Prefer proxy URLs when configured to keep gateway secrets off-device.
...
class CbePaymentGatewayAdapter extends PaymentGatewayAdapter {
  ...
  Future<PaymentGatewayResult> charge(PaymentGatewayRequest request) async {
    ...
    // tx_ref is required by CBE flows and should stay unique per attempt.
  ...
  Map<String, String> _cbeHeaders() {
    // Proxy mode forwards a lightweight bearer token; direct mode needs CBE secret.
...
class UnifiedPaymentGateway {
  ...
  Future<PaymentGatewayResult> charge(PaymentGatewayRequest request) async {
    // Route the request to the adapter that owns the selected payment method.
  ...
  Future<PaymentGatewayResult> verifyWithPolling(PaymentGatewayVerificationRequest request) async {
    ...
    // Poll until gateway stops reporting pending or max attempts is reached.
...
String _randomDigits(int length) {
  // Small helper for generating readable random suffixes.
  final random = Random();
...
Future<Map<String, dynamic>> _postJson({
  ...
  // Shared HTTP + timeout + response-shape handling for all gateway calls.
...
bool _isResponseSuccessful(Map<String, dynamic> response) {
  // Gateways differ in response shape, so check multiple common success markers.
...
PaymentStatus _resolvePaymentStatus(Map<String, dynamic> response) {
  // Normalize many provider-specific status values into app-level PaymentStatus.
...
Map<String, dynamic> _withoutNullOrEmpty(Map<String, dynamic> value) {
  // Prevent sending blank fields that can fail strict gateway validators.
...
String? _firstPresentString(Map<String, dynamic> data, List<String> paths) {
  // Return the first non-empty value from a set of candidate JSON paths.
...
dynamic _readPath(Map<String, dynamic> data, String path) {
  // Supports dotted paths and list indexes (e.g., errors.0.message).
...
Uri _safeUri(String rawValue, String label) {
  // Fail fast with clear messages when endpoints are missing or malformed.
  if (rawValue.isEmpty) {
