...
// AppGateway decides which top-level experience to show: auth, branch selection,
// the admin portal, or the regular shopper shell.
...
class _AppGatewayState extends ConsumerState<AppGateway> {
  ...
  Widget build(BuildContext context) {
              ...
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => ForgotPasswordScreen(
                    onRequestReset: ({required email}) => ref.read(authProvider.notifier).requestPasswordReset(email: email),
                    onConfirmReset: ({required email, required token, required newPassword}) =>
    ...
    if (branchState.selectedBranchId == null) {
      // Branch selection is mandatory because catalog/inventory are branch-scoped.
...
class _AppShellState extends ConsumerState<AppShell> {
  ...
  Future<void> _bootstrapData() async {
    await ref.read(productProvider.notifier).loadAllProducts();
    // Orders/payment options are loaded globally for checkout and profile/order tabs.
  ...
  Widget build(BuildContext context) {
    ...
    final pages = [
      ...
      // Checkout stays inside the cart tab flow, but still walks through address,
      // payment, and final order confirmation as separate screens.
      CartScreen(
        state: cartState,
        ...
        child: Padding(
          ...
          child: DecoratedBox(
            decoration: BoxDecoration(
              ...
              // Index-based shell keeps navigation simple for mobile storefront experience.
            child: NavigationBar(
...
class _AdminPortalShellState extends ConsumerState<AdminPortalShell> {
  ...
  Future<void> _bootstrapData() async {
    ...
    // Admin settings payload powers category/payment/admin-account controls in one request path.
    await ref.read(categoryProvider.notifier).loadCategories();
    await ref.read(paymentProvider.notifier).loadPaymentOptions();
  ...
  Widget build(BuildContext context) {
    ...
    // The shell keeps async data wiring here and passes plain callbacks into the
    // dashboard widget so the UI stays mostly presentational.
    return AdminDashboardScreen(
      ...
      onDeleteProduct: (productId) async {
        // Keep dashboard list fresh after destructive admin actions.
      ...
      onFetchBranch: isSuperAdmin
          ...
          : null,
      onDeleteBranch: isSuperAdmin
          ? (branchId) async {
              await ref.read(adminSettingsProvider.notifier).deleteBranch(branchId);
              await ref.read(branchProvider.notifier).loadBranches();
              final selectedBranchId = ref.read(branchProvider).selectedBranchId;
              if (selectedBranchId != null) {
                await ref.read(productProvider.notifier).loadProducts(branchId: selectedBranchId);
              }
            }
          : null,
      onUpdateBranch: isSuperAdmin
          ...
          : null,
      onAddProduct: () async {
        // Product create flow is delegated to a dedicated form route.
        await Navigator.of(context).push(
          MaterialPageRoute<void>(
            builder: (_) => AddProductScreen(
              categories: categoryState.categories,
              branches: branchState.branches,
              // The form uploads raw bytes first, then stores returned URL in Product.imageUrl.
              onUploadImage: ({required bytes, required fileName}) async {
                return ref.read(commerceApiDataSourceProvider).uploadProductImage(
                      bytes: bytes,
                      fileName: fileName,
                    );
              },
              onSubmit: (product) async {
                ...
                if (isSuperAdmin) {
                  ...
                  // Ensure refresh uses a branch that actually contains the newly added product.
                  final branchNotifier = ref.read(branchProvider.notifier);
                  final currentBranchId = ref.read(branchProvider).selectedBranchId;
                  final preferredBranchId = product.branchIds.isNotEmpty ? product.branchIds.first : currentBranchId;
                  ...
                  if (preferredBranchId != null && preferredBranchId.isNotEmpty) {
                    ...
                    // Reset storefront filters and reload so the new product is visible immediately.
                    await ref.read(productProvider.notifier).searchProducts('');
                    await ref.read(productProvider.notifier).filterByCategory(null);
                    await ref.read(productProvider.notifier).loadProducts(branchId: preferredBranchId);
                  }
                }
                if (context.mounted) {
              ...
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => AdminBranchesScreen(
                    ...
                    onFetchBranch: (branchId) async {
                      return ref.read(adminSettingsProvider.notifier).fetchBranchById(branchId);
                    },
                    onUpdateBranch: ({required branchId, required name, required location, required phoneNumber, required isActive}) async {
                      await ref.read(adminSettingsProvider.notifier).updateBranch(
                            branchId: branchId,
                            name: name,
                            location: location,
      ...
      onOpenCategoriesPage: () {
        Navigator.of(context).push(
          MaterialPageRoute<void>(
            builder: (_) => AdminCategoriesScreen(
              categories: adminSettingsState.categories.isNotEmpty
                  ? adminSettingsState.categories
                  : categoryState.categories,
              onAddCategory: canAddCategory
                  ? ({required name, required description, required imageUrl}) async {
              ...
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => AdminRequestsScreen(
                    adminAccounts: adminSettingsState.adminAccounts,
                    onCreateAdminAccount: ({required name, required email, required password}) async {
                      await ref.read(adminSettingsProvider.notifier).createAdminAccount(
                            name: name,
                            email: email,
                            password: password,
                    ...
                    onFetchAdminAccount: (userId) async {
                      return ref.read(adminSettingsProvider.notifier).fetchAdminAccountById(userId);
                    },
                    onUpdateAdminAccount: ({required userId, required name, required email}) async {
                      await ref.read(adminSettingsProvider.notifier).updateAdminAccount(
                            userId: userId,
                            name: name,
                            email: email,
                    ...
                    onApproveAdmin: ({required userId, required approved}) async {
                      await ref.read(adminSettingsProvider.notifier).approveAdmin(
                            userId: userId,
                            approved: approved,
                          );
      ...
      onOpenPaymentOptionsPage: () {
        Navigator.of(context).push(
          MaterialPageRoute<void>(
            builder: (_) => AdminPaymentOptionsScreen(
