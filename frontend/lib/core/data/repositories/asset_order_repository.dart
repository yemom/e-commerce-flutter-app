...
class AssetOrderRepository implements OrderRepository {
  ...
  Future<List<OrderDto>> _orders() async {
    // Local cache enables deterministic behavior in offline/demo mode.
  ...
  Future<Order> confirmOrder(Order order) async {
    // Simulate backend confirmation by forcing status and prepending latest order.
  ...
  Future<Order> updateOrderStatus({
    ...
    // Fallback create path keeps UI resilient if order list was filtered or stale.
    if (index == -1) {
  ...
  Future<Order> verifyOrderPayment({
    ...
    // Mirror backend mutation of nested payment status inside order payload.
    final list = await _orders();
    final index = list.indexWhere((dto) => dto.id == orderId);
    final item = list[index];
    final updated = OrderDto(
